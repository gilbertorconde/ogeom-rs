...
fn build(model: &mut Model, name: &str, d: &[f64]) -> OgeomResult<Shape> {
    let want = |n: usize| -> OgeomResult<()> {
        if d.len() == n {
            Ok(())
        } else {
            Err(ogeom_err!(
    ...
    let built = match name {
        ...
        other => return Err(ogeom_err!(Construction, "no primitive named '{other}'")),
    };
    Ok(built.shape)
}

/// Print the counts, the measurements and the diagnosis, then export if asked.
fn report(model: &Model, shape: &Shape, name: &str, options: &Options) -> OgeomResult<()> {
    println!("{name}:");
    for kind in [
        ShapeType::Solid,
        ShapeType::Shell,
    ...
    if let Some(path) = &options.view {
        ...
        let bounds = ogeom::algo::shape_bounds(model, shape, TOL)?;
        let camera =
            ogeom_view::Camera::framing(&bounds, ogeom::math::Vector::new(1.0, -1.0, 0.7), TOL)?;
        let image = ogeom_view::render(&mesh, &camera, &ogeom_view::Style::default(), TOL)?;
        std::fs::write(path, image.to_ppm())
            .map_err(|e| ogeom_err!(NotDone, "could not write {path}: {e}"))?;
