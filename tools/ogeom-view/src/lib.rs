...
impl Camera {
    ...
    /// # Errors
    ///
    /// [`OgeomError::Construction`](ogeom_core::OgeomError::Construction) if the bound
    /// is empty, or the direction has no length.
    pub fn framing(bounds: &ogeom_math::Aabb, from: Vector, tol: Tolerances) -> OgeomResult<Self> {
        let (Some(low), Some(high)) = (bounds.low(), bounds.high()) else {
            ogeom_bail!(Construction, "there is nothing to frame");
        };
...
/// # Errors
///
/// [`OgeomError::Construction`](ogeom_core::OgeomError::Construction) if the image size
...
pub fn render(
    ...
    if camera.width == 0 || camera.height == 0 {
        ogeom_bail!(Construction, "an image needs a positive size");
    }
...
mod tests {
    use super::*;
    use ogeom_algo::{make_box, make_sphere, shape_bounds};
    ...
    fn shot(mesh: &Triangulation, bounds: &ogeom_math::Aabb, from: Vector) -> Image {
    ...
    fn a_camera_that_sees_nothing_is_refused() {
        ...
        assert!(Camera::framing(&ogeom_math::Aabb::EMPTY, Vector::Z, T).is_err());
    }
}
