...
| Fitting the traced polyline to a spline | §7 | `trace` returns points and their parameters on both surfaces, to a stated chord tolerance. §7's approximation stage — fitting that polyline to a B-spline within a tolerance — is not written, and it is the *same* problem as adaptive fitting in §6, which is deferred for the same reason: a fit that chooses its own resolution and reports success is the shape of answer that gets trusted. Until then the intersection is a polyline and says so. |
| A published corpus for the intersection gate | §7 | Accuracy and completeness are both instrumented now, and every case they run on is one this project generated. That is the weakness the *Verification* section names: a benchmark over inputs of one's own choosing measures the chooser's imagination as much as the code. What is owed is a corpus from outside — published surface/surface benchmark datasets, and real exchange files once §17 can read them. Until then the gate has two instruments and no independent input, and the numbers should be read that way.
