...
| Completeness as a measured quantity | §7 | The benchmark measures accuracy — every point on both surfaces — and the branch *count* is asserted case by case. What does not exist yet is a completeness instrument: sample one surface, find the points near the other, and check each is covered by some reported branch. Accuracy alone scores a tracer that returns one circle of two at machine precision, and the gate cannot close on accuracy alone. |
