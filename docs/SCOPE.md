...
| `make_prism` over a downward-facing profile | §5 | Found by `check_tessellation` on its first run, and not yet fixed. Sweeping the `+Z` face of a box along `+Z` gives a solid whose mesh agrees with its topology. Sweeping the `-Z` face of the same box along the same vector produces four lateral faces that every one of them fails to triangulate — "the face's boundary enclosed no triangulable region" — while the two end faces are fine. The sweep's own tests only ever used the upward face, so this was uncovered. Reproduction: `make_box`, take the face whose role is `FACE_MIN_Z`, `make_prism` along `+Z`, then `triangulate_face` each lateral face. The boundary ring is the place to look: the profile's edges arrive with composed orientations that differ between the two cases, and the lateral face's pcurves are attached without regard to that. |
| Revolution sweep | §5 | `make_prism` is done; `make_revolution` is not. A draft got as far as correct topology, a closed shell, and each face triangulating correctly on its own — the volume of a revolved square matched Pappus to within the deflection. What it did *not* get right is welding across the seam: the mesh came back with twelve unshared triangle edges, three per lateral face, all of them the first segment of an arc at the seam's position. The diagnosis to resume from is that a face closed in `u` has its `u = 0` and `u = 2pi` boundary vertices land on the same point in space, and the triangles either side of that join are not pairing up. Reverted rather than shipped: the topology looking right is exactly what would make a broken mesh hard to notice later. |
