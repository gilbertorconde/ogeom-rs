...
| Corpus faces that still refuse to triangulate | §9 | Nine corpus parts stay unmeasured. The mesh side now closes what it can: a single ring that winds a periodic direction closes against its own translate (doubly periodic) or the degenerate apex/pole row (cones, spheres), and near-coincident ring points collapse before constraint insertion instead of tripping `TooSmall`. The corpus faces resisted anyway — their rings do not land a clean period out after folding, and ctc_03's `TooSmall` comes from something other than consecutive duplicates — which says the fix is the reader-side seam-and-split *synthesis* the two-ring bands get (building the seam edges and splitting the ring in the model), not a mesh-time reinterpretation. That synthesis is the remaining work. |
...
| PMI beyond the semantic core | §15/§17 | Read and written: dimensional characteristics with values and plus/minus bounds — sizes and locations, linear and angular by their own spellings — geometric tolerances with magnitudes and datum precedence, datum letters. Locations keep their two feature groups through the round trip, and the deeper aspect walk resolves the NIST angle to its faces. Still not carried: presentation PMI (the annotation polylines and planes), tolerance modifiers, composite datum systems (A|B), datum targets. |
| Exchange formats beyond STEP, STL, DXF and native | §17 | DXF writes: R12 polylines on VISIBLE and HIDDEN layers with their linetypes, taking bare polylines so drawings, sections and sketches all serve without the io crate knowing them. The §17 table's remainder — IGES, glTF, OBJ, PLY, 3MF, VRML, DXF *reading* — is not started. STEP runs both directions with assemblies, colours, names and semantic PMI; STL both directions; the native format both directions, document layer included. |
