...
| ~~Smoothness across a fitted closed loop's seam~~ **done** | §6 | `fit_points_closed` fits a loop with the join C1: the last movable control point is eliminated through the clamped end-derivative relation inside the least-squares solve, so the constraint is exact rather than patched on after. Opt-in — the constraint spends shape freedom at the join, so `fit_points` keeps its promise unchanged and loop consumers choose the closed entry. Loop parameter correction gained a trust region: Newton on a poor early-round curve can project a point to the far end of the domain, and on a loop that cliff cascades through the monotonicity repair. |
| ~~Points to a B-spline *surface*~~ **done** | §6 | `fit_surface_grid`: rows then columns on shared knot vectors, parameters fixed (a grid's parameterization is shared property), each pass refined against the worst residual across its whole family, the error measured surface-against-every-input at the end. Scattered data still needs a grid first. |
...
| ~~Exact patches for revolved surfaces~~ **done** | §3 | `revolved_patch` builds the one construction all three needed: the profile converts to its exact rational form, the turn is the exact rational unit circle over the swept angle, and each surface control point is a profile control point rotated by an arc control point with the weights multiplied — exact because rotation is linear in (cos u, sin u) and the tensor product factors. Sphere and torus verify to 1e-12 against their own signed distances; a revolved line verifies against the cylinder it sweeps. Trimmed still refuses (two-directional knot restriction). |
| ~~Sweep surfaces proper~~ **done** | §11 | `make_loft_skinned` skins through many planar sections and `make_pipe_skinned` sweeps a circle along a free-form spine on rotation-minimizing frames (double reflection), both over the grid fit: each row's first sample repeated closes the seam *exactly* through the pinned row ends, border iso-curves come straight off the control net (planar when the sections are, which is what lets the caps be planes), and every pcurve is an iso line. Volumes: the cone-section loft against the frustum at its skin's own error; the S-spine pipe against Pappus within one percent. Closed free-form spines remain unspoken. |
| ~~The general offset rebuild~~ **done** | §11 | One rule serves every element now: a face's displacement constraint at any of its points is planar (its normal there, its amount along it), so vertices solve constraints least-squares and Newton-polish onto the moved surfaces, edges re-derive from the moved pair — lines from constraints with tangent pairs deduped, circles from the analytic pair intersection re-framed on old axes so parameters carry — and partial curved faces rebuild wire-by-wire with exact pcurves. All five analytic surfaces offset. Shelling a filleted part is the pinned proof. Free-form faces remain refused. |
| ~~Collapsed 2D offsets~~ **done** | §11 | The raw offset splits at its self-crossings, every sub-piece stands or falls by the offset's own definition — a true boundary point is a full offset from the source, a collapsed sliver is closer — and the survivors reconnect into loops: one is a wire, several are a compound. Open wires offset into their thick-path outline, capped round or square, a straight path's missing plane synthesized. |
| ~~2D corner blends with curved sides~~ **done** | §10 | The tangent circle's centre lies on each side's offset locus — parallel line or concentric circle — and candidates from the loci's closed-form crossings are qualified GccAna-style: centre on the corner's inner side, both tangency feet on the edges, nearest wins. Chamfer distances on an arc are arc lengths. Free-form sides remain refused. |
...
| ~~Canonical surfaces from a sweep~~ **done** | §9 | `make_prism` recognises a line profile edge perpendicular to the travel and builds the wall as a `PlaneSurface`, with the plane's chart identical to the extrusion chart so every pcurve write serves unchanged — the boolean's plane-to-plane same-domain resolution now sees prism walls. Acceptance: a chamfer melts into a prism wall (`a_prism_wall_is_a_plane_a_chamfer_can_melt_into`). |
| ~~Canonical surfaces from a revolution~~ **done** | §9 | `make_revolution` recognises a profile line perpendicular to the axis and builds the face it sweeps on the plane it actually turns in — a disc, annulus or pie bounded by its rails (and, for a partial turn, the profile and its displaced copy), with exact Cartesian pcurves, no seam and no degenerate centre. A revolved rectangle now matches `make_cylinder` edge for edge and vertex for vertex, and the test asserts it. |
...
| ~~Pcurves for edges on B-spline surfaces~~ **done** | §9/§17 | Projection fitting at the curve's own parameters serves every corpus face now: the reader accepts an edge sitting up to `confusion*1e6` from its surface — the corpus's own slop is 1–3 µm, a wrong pairing misses by millimetres — and *records* the measured deviation by widening the edge's tolerance, the same honesty the vertex ends get. Two parts measure (ftc_09 exactly closed, ftc_11), and the parts still refusing do so for face-level reasons below, not for missing pcurves. |
| ~~Welding the cut-imported result~~ **done** | §8/§9 | The mesh welds in three passes now: edge ends anchor to their *vertices* (the one authority every meeting curve shares), each face's chart points fold onto the periodic branch that continues the ring being walked (a cylinder split into halves needs the shared ruling at `2π`, not `0`), and after the confusion-scale weld a border-only pass welds and T-junction-stitches within the tolerance the model recorded (floored at a tenth of the caller's chord — below the resolution they accepted). The imported cut measures: 6730.103 → 6606.903 mm³. |
