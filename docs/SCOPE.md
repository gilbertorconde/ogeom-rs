...
- A multi-body STEP assembly imported with colours, names and PMI, modified,
  ...
  *Residuals:* PMI beyond the semantic core and §15's transactional
  ...
  and undo/redo are in; presentation PMI and datum targets remain, and land
  with M11.
- A 2D drawing generated from a 3D model: hidden lines removed, visible and
  ...
  *Residuals:* the exact path draws the elementary surfaces' silhouettes and
  classifies by ray/surface interference, but a torus or a spline silhouette
  still needs marching — deferred table.
...
was. A touch at a *point* is carried too, on the same reasoning
as a touch along a curve: it bounds nothing, so a ball resting on a lid
fuses into both volumes joined where they meet and cuts nothing at all.
What §8 still owes is named in the deferred table: a section running
exactly through a pole, a ball on a box's corner vertex, the shell around
a three-cylinder tip, and scaled placements.
...
against a parity oracle that asks the solid itself. `manufacturing_plan()`
reads that tree as operations: a bore is a drill of its own diameter along
its own axis, through or blind as recognized; a pocket is a mill with the
depth its walls stand and the largest end mill its corners admit, or none
where the corners are sharp and no round tool leaves that shape; a boss is
material to clear around; a blend is the radius a tool leaves. The order is
the tree's, because a hole through a pocket floor cannot be drilled before
the pocket exists. And `remove_feature()` undoes what a feature *is*: a
hole is filled by the bore it is, a pocket by the prism its floor sweeps, a
boss shaved by the same prism the other way — while a fillet or a chamfer
is refused by name, since undoing one is the wedge construction backwards
and belongs with the blends.
...
| ~~Non-manifold topology (§4)~~ **done** | §4 | Built and pinned: a T-joint of three sheets sharing one spine edge goes through the ordinary builders, traversal counts the spine once and finds it under all three faces, `is_shell_closed` answers false — an edge used three times bounds no volume — and the mesh layer meshes each leaf. A compound mixing a solid, the sheets, a bare edge and a loose vertex traverses by type and round-trips through the native format byte-stable, the sharing intact on the far side. What non-manifold input *means* to each query is now what the tests say it means. |
...
| ~~A consumer for the half-space~~ **done** | §8 | The boolean consumes it: a half space entering cut, common or section is resolved into a box filling its material side, sized past the other argument's whole reach — the box's plane-side face is *coplanar with the boundary itself*, so the cut the caller sees is the exact plane, and the far faces never appear in the result. A block cut by the half space above z = 4 measures its slab exactly; common keeps the cap; and the union refuses as the unbounded thing it would be, by name. |
| Curved regions in mesh → B-rep | §9 · narrowed | Recognition-driven segmentation: triangles no planar region claims grow into regions that must keep recognizing as *one* canonical surface — kind-locked, because a nearly planar sample set also fits a giant cylinder — with per-kind sample floors, interior-first seeding, curvature-spanning bootstraps for curved seeds, and orphan adoption gated by the recognition tolerance, which should be the mesh's own deflection: looser and least-squares strip-planes halve the sagitta and sneak in. A drum's mesh comes back as two caps and one recognized cylinder wall at its exact radius, the shell closed. Still owed, by name: tessellating the rebuilt *full-period* face (the seam class the corpus bands own), and segmentation across tangent-smooth junctions, where a fillet band and its wall share a tolerance-ambiguous strip. |
| ~~Branch points in the marching intersector~~ **done** | §7 | Near-tangent stall ends cluster into branch points; every fragment is cut at its branch-point visits — trimming the wander a failing correction parks there and separating the arcs a walk-through glued together — debris and duplicate coverage drop, and arc ends pair at each branch point by tangent continuation, because a smooth curve crosses its own singularity collinearly while the crossing curve turns. The chains close into loops with the branch point inserted at each junction. Two equal perpendicular cylinders come back as exactly two Closed ellipses measuring 7.6402 against the series value 7.6404. Tangential-contact debris cannot participate: only fragments transversal somewhere in their interior are stitched. |
| Boolean coverage beyond the analytic pairs | §8 · narrowed | The pipeline runs on any faces §7's intersectors answer for, through one general path — the drilled box is the pinned proof — and same-domain contact, planar and curved, unifies rather than refusing. Tangential contact is now routed rather than refused: it is recognised, kept out of the classification, and reported by `section`. Two things the arrangement learnt with it: a chart's *poles* are boundary strands like any other — a degenerate edge is a point in space and a line in the chart, and leaving it out left a sphere's chart with no top or bottom, which is why every boolean over a ball used to fail wherever the ball was — and a piece is classified at more than one interior probe, because a piece that merely touches the other solid has its roomiest probe sitting on that contact. Still named: a section running exactly *through* a pole (a plane cutting a ball on its own axis) leaves pieces that do not close into a shell; a sphere seated on a box's corner vertex fails in the rebuild rather than by name; and scaled placements refuse because a scale changes a surface's parameterization out from under its pcurves. |
| ~~Same-domain faces in the boolean~~ **done** | §8 | Planar pairs split each other by the other face's boundary edges and resolve On pieces by comparing outward normals, as before. The refusals the entry still named turned out to be paid by the machinery that grew after it was written: coaxial cylinders sharing one surface fuse flush, overlap and cut to their closed forms — the rim, seam and now trig pcurves carry every boundary into the shared chart — and contact confined to an edge or a vertex passes through the pipeline whole, the edge-touching fuse one valid solid of both volumes with its non-manifold seam, the corner-touching cut removing nothing. All five pinned in `boolean_contact.rs`. |
| ~~Tangential contact along a curve~~ **done** | §7/§8 | The walker follows the valley of the gap function — step along the common tangent plane, correct by closing the gap between mutual projections, a minimization rather than a root-find because at tangency the gap touches zero without crossing — and it is now *reached* without being asked for. `intersect_surfaces` marks every section curve as crossing or touching: the analytic path decides it by reading both surfaces' normals along the curve through its own pcurves, and the marched path hands its stalled fragments to the walker instead of discarding them, one walk per contact rather than one per fragment. A plane resting on a torus comes back as the contact circle at radius 3, closed; a ball seated in a torus tube, which nothing analytic answers, comes back as arcs whose ends are the ball's own chart poles and which together measure the tube circle. The boolean keeps contact out of its arithmetic, which is what tangency means, and `section` reports it as the edge it is — clipped to where both faces reach it, decided by sampling. `tangency_along_a_circle_produces_fragments_not_a_curve` still pins what the *crossing* walker alone can say. |
| ~~Crossings shallower than a microradian~~ **done** | §7 | The gate is decided by the interval filter rather than a bare compare: each normal component carries the correction's stated residual as an uncertainty, the squared cross magnitude is computed as an enclosure, and only a crossing *certainly* above the floor is followed. A sine inside the enclosure's undecided band is exactly what the floor exists for — the correction's own noise masquerading as an angle — and it is refused with a certificate instead of a guess. Identical spheres still yield nothing, now provably rather than by threshold. |
...
| ~~Restriction of closed exact sections to surface extents~~ **resolved as design** | §8 | Section *lines* are clipped to both surfaces' parameter extents through their exact pcurves; a closed curve partially outside is kept whole, and that is the design rather than a debt: the restriction that matters is the face's trim, which is exactly the boolean's 2D splitting stage — the same-domain and contact tests exercise it — while the surface extent is only the parameterization window. Cutting at the window would split a curve where no face boundary exists. |
...
| Vertex blends, marching blends | §10 · narrowed | Face-face blends and blend analysis are done — the first as a seat found from the two planes' own intersection rather than from an edge, the second as `analyse_blend`, which reports the largest normal angle and the largest curve-to-surface gap per shared edge instead of asserting either. What remains: **the marching blend** — edges whose envelope has no closed form, where the spine is the intersection of the two offset surfaces and the sections are circular arcs skinned along it, every part of which exists separately — and **the setback vertex blend**. The vertex construction itself turned out to be simple and is written down here rather than lost: after the three edge fillets, the corner block from the ball's centre out past the corner, *minus the ball*, is exactly the leftover spike — anything in that block further from the centre than the ball lies outside one of the three fillet cylinders and was already cut away. What defeats it is the removal, not the tool: the ball meets the three fillet cylinders tangentially and they meet each other at a three-cylinder tip, and the boolean answers `the kept pieces did not close into a shell`. Not tangency alone — a ball a percent under the radius, touching nothing, fails the same way. |
