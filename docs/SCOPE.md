...
| Welding the healed part's mesh | §9 | Ring re-anchoring makes the smallest NIST part topologically whole: every face meshes and the shell closes by edge count. The face meshes do not yet weld along the re-anchored rings — adjacent faces sample the shared boundary at densities that differ near the tolerance-widened vertices — so the part will not measure a volume until the welding agrees. The healer's structure is right; the mesh boundary sampling contract is the remaining work. |
