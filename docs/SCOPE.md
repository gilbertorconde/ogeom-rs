...
| Boolean coverage beyond the analytic pairs | §8 · M8 | The pipeline runs on any faces §7's intersectors answer for, through one general path — the drilled box is the pinned proof, and same-domain contact, planar and curved, now unifies rather than refusing. What remains named: tangential contact refuses where the touch lies within both faces' reach — the tangential walker exists and M8 owes the *routing* — and scaled placements refuse because a scale changes a surface's parameterization out from under its pcurves. The refusal gates stay generous by design. |
...
| Tangential contact along a curve | §7 · M8 routes it | The walker exists: `trace_tangential` follows the valley of the gap function — step along the common tangent plane, correct by closing the gap between mutual projections, a minimization rather than a root-find because at tangency the gap touches zero without crossing. A unit sphere seated in a unit cylinder traces its full equator, Closed, length 2π to a hundredth. The crossing walker keeps its honest refusal (`tangency_along_a_circle_produces_fragments_not_a_curve` still pins it); what remains is *routing* — the boolean and the section pipeline recognising a tangential configuration and calling this walker — which is M8's contact work. |
