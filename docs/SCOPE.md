...
| Smoothness across a fitted closed loop's seam | §6 | `approximate_within` and the fitting core under it are done: an error target in, knots refined where the error concentrates, parameter correction so the residual measures the curve rather than the parameterization, and the error reached reported with a `met` flag that does not round up. What remains is the seam of a *closed* fit: the first and last points are honoured exactly so a loop closes, but only with positional continuity — the tangents either side of the join are not constrained to agree. A boolean sees a closed edge either way; a surface built over the curve would see the crease. |
