...
| A published corpus for the intersection gate | §7 | The corpus is live: §17's reader imports all eleven NIST parts — every solid found, seven of eleven shells closing on first read, the rest reporting exactly which faces were compromised and why. The instruments now have outside input; what remains is *using* it — healing over the corpus, booleans over imported parts — which is the M3 criterion the corpus was fetched for. |
