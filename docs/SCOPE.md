...
The **vertical slice is in**: the whole pipeline, end to end, for planar-faced
solids with straight edges. Sections are exact plane/plane lines clipped to
both trims, faces split in their own parameter planes by an exact arrangement,
pieces classified by the exact ray classifier, results sewn, closure demanded,
shells nested into solids and voids, history recorded. Fuse, common, cut and
section all select from the one general result. Curved geometry and
same-domain contact are refused with instructions — see the deferred table.
...
| Curved geometry in the boolean | §8 | The vertical slice runs the whole pipeline — sections, arrangement, classification, sewing, nesting, history — for planar-faced solids with straight edges, and *refuses* a curved face or edge by name. Everything the curved case needs already exists in §7: `intersect_surfaces` hands back section curves with same-parameter pcurves on both faces, and the classification is already exact for every analytic surface. What widening the slice takes is paves living on curves rather than in planes — the indexed interference data structure — and face splitting in a parameter space where boundaries are pcurves rather than polygons. |
| Same-domain faces in the boolean | §8 | A piece of one argument's face lying *on* the other argument's boundary — coplanar overlapping walls, a box stacked exactly on a box — is refused, because keeping it once, twice, or not at all are three different answers and the right one depends on which operation is running and which sides the two solids occupy. The conventional kernel unifies same-domain faces in a dedicated builder stage; that stage lands with the widened slice. The classifier already reports the configuration honestly as `On`, which is what the refusal keys on. |
...
| A published corpus for the intersection gate | §7 | Accuracy and completeness are both instrumented, and the first outside input has arrived: `tests/hard.rs` transcribes configurations the surface/surface literature names as hard — equal cylinders at a shallow angle, near-tangent quadrics, a sphere across a cone's apex — and the transcription earned its keep at once by catching `Cone::distance_to` measuring one nappe of a two-nappe surface. But transcribed configurations are still constructions; no published surface/surface benchmark *dataset* exists to import. What remains owed is real exchange files once §17 can read them: `tests/corpus/` stands ready with its manifest rules, and the NIST MBE PMI models are the vetted source. Until then the numbers should be read as instruments plus literature, not instruments plus the world. |
