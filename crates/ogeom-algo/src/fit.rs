...
//! reached. The machinery lives in [`ogeom_geom::fit`], where the intersector's
...
/// be met with the points given, [`Fitted::met`](ogeom_geom::fit::Fitted) says so
...
pub fn approximate_within(
    ...
    tolerance: f64,
    tol: Tolerances,
) -> OgeomResult<ogeom_geom::fit::Fitted<BSplineCurve>> {
    ogeom_geom::fit::fit_points(points, 3, tolerance, tol)
}
...
/// # Errors
///
/// [`OgeomError::Construction`](ogeom_core::OgeomError::Construction) if there are too
...
fn parameterize(points: &[Point], spacing: Spacing, tol: Tolerances) -> OgeomResult<Vec<f64>> {
    let n = points.len();
    if n < 2 {
        ogeom_bail!(Construction, "fitting needs at least two points");
    }
...
fn spread(degree: usize, control_count: usize, parameters: &[f64]) -> OgeomResult<KnotVector> {
...
fn solve(matrix: &nalgebra::DMatrix<f64>, rhs: &[Point]) -> OgeomResult<Vec<Point>> {
    let n = rhs.len();
