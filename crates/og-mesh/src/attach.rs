...
use og_topo::{EdgeRepr, Filter, Model, NodeData, Shape, ShapeType, Triangulation, explore_unique};
...
pub fn tessellate(
    ...
    for face in og_topo::explore(model, shape, Filter::OfType(ShapeType::Face))? {
...
#[must_use]
pub fn polyline_of(model: &Model, edge: &Shape) -> Option<(Vec<og_math::Point>, Vec<f64>)> {
...
mod tests {
    ...
    fn the_cached_boundary_agrees_with_the_cached_faces() {
        ...
        for edge in explore_unique(&model, &built.shape, ShapeType::Edge).unwrap() {
            ...
            for face in og_topo::ancestors_of(&model, &built.shape, &edge, ShapeType::Face).unwrap()
            {
    ...
    fn a_shape_with_no_faces_tessellates_to_nothing_rather_than_failing() {
        let mut model = Model::new();
        let vertex = model.add_point(og_math::Point::ORIGIN);
