...
impl Deflection {
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if `chord` is
    /// not finite and positive.
    pub fn with_chord(chord: f64) -> OgResult<Self> {
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if `size` or
    /// `fraction` is not finite and positive.
    pub fn relative(size: f64, fraction: f64) -> OgResult<Self> {
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if a tolerance
    ...
    pub fn validate(&self) -> OgResult<()> {
        ...
        if self.min_segments == 0 {
            og_bail!(Construction, "a polyline needs at least one segment");
        }
...
/// [`OgError::Domain`](og_core::OgError::Domain) if the range leaves the curve.
pub fn discretize(
    curve: &Curve,
    range: (f64, f64),
...
fn needs_split(
    ...
    let chord = og_math::Axis::through(ends.0, ends.1, tol).map_or_else(
...
pub fn discretize_planar(
    curve: &og_geom::PlanarCurve,
    range: (f64, f64),
    deflection: Deflection,
    tol: Tolerances,
) -> OgResult<(Vec<og_math::Point2>, Vec<f64>)> {
    use og_geom::Curve2d;
    ...
    let mut parameters: Vec<f64> = (0..=deflection.min_segments)
        ...
        .collect();
    let mut points: Vec<og_math::Point2> = parameters
        .iter()
        .map(|u| curve.point_at(*u, tol))
        .collect::<OgResult<_>>()?;
    ...
    while points.len() <= deflection.max_segments {
        ...
        for i in 0..points.len() - 1 {
            let mid = f64::midpoint(parameters[i], parameters[i + 1]);
            let on_curve = curve.point_at(mid, tol)?;
            let chord = og_math::Axis2::through(points[i], points[i + 1], tol).map_or_else(
...
mod tests {
    ...
    use og_geom::{BSplineCurve, CircleCurve, LineCurve};
    use og_math::{Circle, Frame, KnotVector};
    ...
    fn worst_error(curve: &Curve, line: &Polyline) -> f64 {
        ...
        for window in line.parameters.windows(2) {
            for k in 1..16 {
                ...
                let chord =
                    og_math::Axis::through(a, b, T).map_or(0.0, |axis| axis.distance_to(on_curve));
    ...
    fn a_planar_curve_discretizes_in_parameter_space() {
        let curve: og_geom::PlanarCurve = og_geom::Circle2d::new(
            og_math::Circle2::centred(og_math::Point2::ORIGIN, 5.0, T).unwrap(),
        )
        .into();
