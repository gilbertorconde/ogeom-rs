...
fn l_bracket(model: &mut og_topo::Model) -> (og_topo::Shape, og_topo::Shape) {
    ...
    .unwrap();
    let notch = og_algo::make_box(model, seat, (2.0, 3.0, 2.0), T).unwrap();
    let cut = og_bool::cut(model, &block.shape, &notch.shape, T).unwrap();
...
fn a_concave_chamfer_fills_the_corner_with_its_bevel() {
    ...
    let d = 0.25;
    let result = og_fillet::chamfer_edge(&mut model, &bracket, &edge, d, T).unwrap();
...
fn a_hole_rim_gains_the_mirrored_toroidal_blend() {
    let mut model = og_topo::Model::new();
    let outer = og_algo::make_cylinder(&mut model, Frame::WORLD, 2.0, 2.0, T).unwrap();
    let bore = og_algo::make_cylinder(&mut model, Frame::WORLD, 1.0, 2.0, T).unwrap();
    let tube = og_bool::cut(&mut model, &outer.shape, &bore.shape, T).unwrap();
    ...
    let r = 0.3;
    let result = og_fillet::fillet_edge(&mut model, &tube.shape, &edge, r, T).unwrap();
    let diagnosis = og_algo::check(&model, &result.shape, T).unwrap();
    assert!(diagnosis.is_valid(), "{:?}", diagnosis.problems);
...
fn a_boss_base_gains_an_additive_toroidal_blend() {
    let mut model = og_topo::Model::new();
    let plate = og_algo::make_box(&mut model, Frame::WORLD, (10.0, 10.0, 2.0), T).unwrap();
    let seat = Frame::new(
    ...
    .unwrap();
    let boss = og_algo::make_cylinder(&mut model, seat, 1.0, 3.0, T).unwrap();
    let joined = og_bool::fuse(&mut model, &plate.shape, &boss.shape, T).unwrap();
    ...
    let r = 0.3;
    let result = og_fillet::fillet_edge(&mut model, &joined.shape, &edge, r, T).unwrap();
    let diagnosis = og_algo::check(&model, &result.shape, T).unwrap();
    assert!(diagnosis.is_valid(), "{:?}", diagnosis.problems);
