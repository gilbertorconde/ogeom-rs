...
use og_topo::{Location, Model, Shape, Triangulation};
...
/// # Errors
///
/// [`OgError::Construction`](og_core::OgError::Construction) if the mesh names
/// a vertex it does not have, or `crease` is not a positive angle.
pub fn planar_regions(
    mesh: &Triangulation,
    crease: f64,
    tol: Tolerances,
) -> OgResult<(Vec<Region>, Vec<usize>)> {
    if !crease.is_finite() || crease <= 0.0 {
        og_bail!(Construction, "a crease angle of {crease} names no edge");
    }
...
/// # Errors
///
/// [`OgError::Construction`](og_core::OgError::Construction) if the mesh is
...
pub fn to_brep(
    ...
    if mesh.triangles.is_empty() {
        og_bail!(Construction, "there are no triangles to rebuild from");
    }
    ...
    for region in &regions {
        let surface = model
            .geometry_mut()
            .add_surface(og_geom::PlaneSurface::new(region.plane).into());
        ...
        for ring in &region.loops {
            ...
            for i in 0..ring.len() {
                ...
                for (index, at) in [(from, a), (to, b)] {
                    vertices
                        .entry(index)
                        .or_insert_with(|| model.add_vertex(og_topo::VertexData::new(at)));
                }
                ...
                let edge = match shared.get(&key) {
                    ...
                    None => {
                        let fresh = crate::build::make_edge_between(
                            model,
                            og_geom::LineCurve::segment(a, b, tol)?.into(),
                            (0.0, a.distance(b)),
                            &vertices[&from].clone(),
                            &vertices[&to].clone(),
                            tol,
                ...
                let (pa, pb) = if edge.orientation() == og_topo::Orientation::Reversed {
                    (b, a)
                } else {
                    (a, b)
                };
...
fn attach(
    ...
    crate::build::attach_pcurve(
        model,
        edge,
        og_geom::Line2d::segment(a, b, tol)?.into(),
        surface,
        Location::identity(),
        (0.0, a.distance(b)),
    )
...
mod tests {
    ...
    fn a_box_survives_the_round_trip_through_a_mesh() {
        ...
        assert_eq!(
            ...
            og_topo::ShapeType::Solid,
            "a closed mesh should come back a solid"
        );
        let counts = |kind| {
            og_topo::explore_unique(&model, &recovered.shape, kind)
                .unwrap()
                .len()
        };
        assert_eq!(counts(og_topo::ShapeType::Face), 6);
        assert_eq!(counts(og_topo::ShapeType::Edge), 12);
        assert_eq!(counts(og_topo::ShapeType::Vertex), 8);
    ...
    fn two_coplanar_patches_that_do_not_touch_are_two_regions() {
        ...
        let there = crate::transformed(
            &mut model,
            &here.shape,
            og_math::Transform::translation(Vector::new(5.0, 0.0, 0.0)),
        )
        .unwrap();
    ...
    fn an_empty_or_broken_mesh_is_refused() {
        ...
        let _ = shape_bounds(&model, &Shape::of(og_core::Key::from_parts(0, 0)), T);
    }
}
