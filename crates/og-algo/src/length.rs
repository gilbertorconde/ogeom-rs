...
//!
//! [`og_mesh::discretize()`] places points where the curve *bends*,
...
use og_math::{Point, integrate, solve};
...
/// # Errors
///
/// [`OgError::Domain`](og_core::OgError::Domain) if `range` is not finite;
/// [`OgError::NotDone`](og_core::OgError::NotDone) if the integral does not
/// converge, which means the parameterization is singular somewhere in the
/// range rather than that the curve is long.
pub fn curve_length(curve: &Curve, range: (f64, f64), tol: Tolerances) -> OgResult<f64> {
    let (lo, hi) = range;
    if !lo.is_finite() || !hi.is_finite() {
        og_bail!(Domain, "cannot measure the length of [{lo}, {hi}]");
    }
...
/// # Errors
///
/// [`OgError::Domain`](og_core::OgError::Domain) if `target` is negative or
/// past the end; [`OgError::NotDone`](og_core::OgError::NotDone) if the solve
/// does not converge.
...
/// # Errors
///
/// [`OgError::Construction`](og_core::OgError::Construction) if fewer than two
...
/// # Errors
///
/// [`OgError::Construction`](og_core::OgError::Construction) if `spacing` is
...
mod tests {
    ...
    fn length_is_measured_on_the_curve_not_on_a_polyline_through_it() {
        ...
        let mesh = og_mesh::discretize(&c, (0.0, TAU), og_mesh::Deflection::default(), T).unwrap();
        assert!(
