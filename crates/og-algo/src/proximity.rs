...
//! extrema in `og-intersect` answer where two curves or surfaces come
...
use og_geom::{Curve, SurfaceGeometry, Transformable, TrimmedCurve};
use og_math::{Point, Point2, Transform};
use og_mesh::{Deflection, face_boundary, inside_boundary};
use og_topo::{EdgeRepr, Model, NodeData, Shape, ShapeType, explore_unique};

use crate::classify::{distance_to_rings, parametric_band};
use crate::measure::{project_on_curve, project_on_surface};
use og_intersect::ExtremaOptions;
...
/// # Errors
///
/// [`OgError::Construction`](og_core::OgError::Construction) if either shape
...
pub fn distance_between_shapes(
    ...
    tol: Tolerances,
) -> OgResult<ShapeDistance> {
    let ea = elements(model, a, tol)?;
    let eb = elements(model, b, tol)?;
    if ea.is_empty() || eb.is_empty() {
        og_bail!(Construction, "a shape with no elements has no distance");
    }
...
fn approach(
    ...
    match (a, b) {
        ...
        (Element::Edge(sa, ca), Element::Edge(sb, cb)) => {
            let found = og_intersect::extrema_curve_curve(ca, cb, options, tol)?;
            for near in &found.approaches {
                push(near.distance, near.point_a, near.point_b, sa, sb);
            }
        }
        (Element::Edge(sa, curve), Element::Face(sb, face)) => {
            let found = og_intersect::extrema_curve_surface(curve, &face.world, options, tol)?;
            for near in &found.approaches {
        ...
        (Element::Face(sa, fa), Element::Face(sb, fb)) => {
            let found = og_intersect::extrema_surface_surface(&fa.world, &fb.world, options, tol)?;
            for near in &found.approaches {
...
fn inside_trim(face: &Prepared, world_point: Point, tol: Tolerances) -> OgResult<bool> {
...
fn elements(model: &Model, shape: &Shape, tol: Tolerances) -> OgResult<Vec<Element>> {
    let mut out = Vec::new();
...
fn restrict_to_rings(
    ...
    Ok(og_geom::TrimmedSurface::new(surface.clone(), (lo_u, hi_u), (lo_v, hi_v), tol)?.into())
}
...
mod tests {
    ...
    use og_math::{Direction, Frame, Vector};
