...
pub fn fit_points_closed(
    ...
    let Some((first, last)) = points.first().zip(points.last()) else {
        ogeom_bail!(Construction, "a closed fit needs points");
    };
...
/// # Errors
///
/// [`OgeomError::Construction`](ogeom_core::OgeomError::Construction) if the grid is
...
pub fn fit_surface_grid(
    ...
    let nv = rows.len();
    if nv < 2 {
        ogeom_bail!(Construction, "a surface fit needs at least two rows");
    }
    let nu = rows[0].len();
    ...
    let grid = ogeom_math::ControlGrid::new(net, k, l)?;
...
fn single_span(degree: usize, a: f64, b: f64) -> OgeomResult<KnotVector> {
