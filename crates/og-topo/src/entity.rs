...
use og_core::{Arena, Key, OgResult, Tolerance, Tolerances, og_bail};
...
impl VertexData {
    ...
    pub fn with_tolerance(point: Point, tolerance: f64) -> OgResult<Self> {
        if !point.is_finite() {
            og_bail!(Construction, "vertex position is not finite");
        }
        Ok(Self {
            point,
...
/// # Errors
///
/// [`OgError::Invariant`](og_core::OgError::Invariant) if `bounding` is tighter
/// than `bounded`.
pub fn check_containment(bounding: Tolerance, bounded: Tolerance) -> OgResult<()> {
    og_core::check_containment(bounding, bounded)
}
...
/// # Errors
///
/// [`OgError::Construction`](og_core::OgError::Construction) if the range is
/// empty or non-finite.
pub fn check_range(range: (f64, f64), tol: Tolerances) -> OgResult<()> {
    let (a, b) = range;
...
mod tests {
    ...
    use og_math::{Circle, Direction, Frame, Plane};
    ...
    fn store() -> (GeometryStore, CurveId, SurfaceId, PCurveId) {
        ...
        let pcurve = s.add_pcurve(
            og_geom::Line2d::segment(og_math::Point2::ORIGIN, og_math::Point2::new(10.0, 0.0), T)
                .unwrap()
                .into(),
        );
    ...
    fn an_edge_holds_several_representations_at_once() {
        ...
        let other_pcurve = s.add_pcurve(
            og_geom::Line2d::segment(og_math::Point2::ORIGIN, og_math::Point2::new(0.0, 10.0), T)
                .unwrap()
                .into(),
        );
    ...
    fn a_seam_carries_two_pcurves_because_one_cannot_express_both_sides() {
        ...
        let cylinder = s.add_surface(
            og_geom::CylinderSurface::new(
                og_math::Cylinder::new(Frame::WORLD, 2.0, T).unwrap(),
                (0.0, 5.0),
            )
            .unwrap()
            .into(),
        ...
        let at_zero = s.add_pcurve(
            og_geom::Line2d::segment(og_math::Point2::ORIGIN, og_math::Point2::new(0.0, 5.0), T)
                .unwrap()
                .into(),
        );
        let at_tau = s.add_pcurve(
            og_geom::Line2d::segment(
                og_math::Point2::new(core::f64::consts::TAU, 0.0),
                og_math::Point2::new(core::f64::consts::TAU, 5.0),
                T,
            )
            .unwrap()
            .into(),
