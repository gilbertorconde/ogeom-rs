...
use ogeom_core::{Arena, Key, OgeomResult, Tolerances, ogeom_bail};
use ogeom_math::{Transform, TransformKind};
...
impl Location {
    ...
    /// # Errors
    ///
    /// [`OgeomError::Dangling`](ogeom_core::OgeomError::Dangling) if the chain names a
    /// datum the store does not hold, and
    /// [`OgeomError::Numeric`](ogeom_core::OgeomError::Numeric) if a negative power
