...
impl Quaternion {
    ...
    /// # Errors
    ///
    /// [`OgeomError::Construction`](ogeom_core::OgeomError::Construction) if the two are
    ...
    /// # Errors
    ///
    /// [`OgeomError::Construction`](ogeom_core::OgeomError::Construction) if `m` is not
    ...
    pub fn from_matrix(m: &Matrix3, tol: Tolerances) -> OgeomResult<Self> {
        if !m.is_orthonormal(ORTHONORMAL_EPS) {
            ogeom_bail!(Construction, "rotation matrix is not orthonormal");
        }
        if m.determinant() < 0.0 {
            ogeom_bail!(Construction, "matrix is a reflection, not a rotation");
        }
        let r = &m.rows;
        let trace = m.trace();
        let q = if trace > 0.0 {
    ...
    /// # Errors
    ///
    /// [`OgeomError::Construction`](ogeom_core::OgeomError::Construction) if the norm is
    /// below `tol.confusion()` or any component is non-finite.
    pub fn normalized(self, tol: Tolerances) -> OgeomResult<Self> {
        if !self.is_finite() {
            ogeom_bail!(Construction, "cannot normalize a non-finite quaternion");
        }
        let n = self.norm();
        if n <= tol.confusion() {
            ogeom_bail!(Construction, "cannot normalize a quaternion of norm {n}");
        }
    ...
    /// # Errors
    ///
    /// [`OgeomError::Numeric`](ogeom_core::OgeomError::Numeric) if the norm is
    /// degenerate.
    pub fn inverse(self, tol: Tolerances) -> OgeomResult<Self> {
        let n2 = self.square_norm();
        if n2 <= tol.confusion() * tol.confusion() {
            ogeom_bail!(Numeric, "quaternion of norm {} has no inverse", n2.sqrt());
        }
    ...
    /// # Errors
    ///
    /// [`OgeomError::Construction`](ogeom_core::OgeomError::Construction) if the rotation
    ...
    pub fn to_axis_angle(self, tol: Tolerances) -> OgeomResult<(Direction, f64)> {
        ...
        if sin_half <= tol.confusion() {
            ogeom_bail!(Construction, "identity rotation has no defined axis");
        }
