...
impl CurvePoint {
    ...
    pub fn tangent(&self, tol: Tolerances) -> OgeomResult<Direction> {
...
pub fn circle_parameter(circle: &Circle, p: Point, tol: Tolerances) -> OgeomResult<f64> {
...
/// # Errors
///
/// [`OgeomError::Construction`](ogeom_core::OgeomError::Construction) if `p` projects to
/// the centre.
pub fn ellipse_parameter(ellipse: &Ellipse, p: Point, tol: Tolerances) -> OgeomResult<f64> {
    let local = ellipse.frame().to_local(p);
    if local.x.hypot(local.y) <= tol.confusion() {
        ogeom_bail!(Construction, "point projects to the ellipse's centre");
    }
...
/// # Errors
///
/// [`OgeomError::Domain`](ogeom_core::OgeomError::Domain) if `p` projects to the far
/// branch, which this hyperbola does not describe.
pub fn hyperbola_parameter(hyperbola: &Hyperbola, p: Point, tol: Tolerances) -> OgeomResult<f64> {
...
pub fn sphere_parameters(sphere: &Sphere, p: Point, tol: Tolerances) -> OgeomResult<(f64, f64)> {
    let local = sphere.frame().to_local(p);
    let ring = local.x.hypot(local.y);
    if ring.hypot(local.z) <= tol.confusion() {
        ogeom_bail!(Construction, "point is the sphere's centre");
    }
...
pub fn torus_parameters(torus: &Torus, p: Point, tol: Tolerances) -> OgeomResult<(f64, f64)> {
