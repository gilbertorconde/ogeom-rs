...
//! `BRepExtrema` there and lives in `og-algo` here — this module answers for
...
/// are unusable; [`OgError::Domain`](og_core::OgError::Domain) if a curve's
...
pub fn extrema_curve_curve(
    ...
    tol: Tolerances,
) -> OgResult<Extrema<f64, f64>> {
    if options.samples < 2 {
        og_bail!(Construction, "seeding needs at least two samples");
    }
    ...
    if sa.len() < 2 || sb.len() < 2 {
        og_bail!(Construction, "a curve failed to evaluate over its domain");
    }
...
fn line_line(a: &og_geom::LineCurve, b: &og_geom::LineCurve, tol: Tolerances) -> Extrema<f64, f64> {
...
pub fn extrema_surface_surface(
    ...
    options: ExtremaOptions,
    tol: Tolerances,
) -> OgResult<Extrema<(f64, f64), (f64, f64)>> {
    if options.grid < 2 {
        og_bail!(Construction, "seeding needs at least two steps each way");
    }
    ...
    if sa.is_empty() || sb.is_empty() {
        og_bail!(Construction, "a surface failed to evaluate over its domain");
    }
...
fn wide_surface_check(surface: &SurfaceGeometry) -> OgResult<()> {
...
mod tests {
    use super::*;
    use og_geom::{CircleCurve, CylinderSurface, LineCurve, PlaneSurface, SphereSurface};
    use og_math::{Circle, Cylinder, Direction, Frame, Plane, Sphere};
    ...
    fn an_untrimmed_line_is_refused_with_instructions() {
        let endless: Curve = LineCurve::new(og_math::Axis {
        ...
        let other: Curve = LineCurve::new(og_math::Axis {
