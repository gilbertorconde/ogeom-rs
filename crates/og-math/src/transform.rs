...
impl Transform {
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if `factor` is
    ...
    /// degenerate geometry.
    pub fn scaling(centre: Point, factor: f64, tol: Tolerances) -> OgResult<Self> {
        if !factor.is_finite() || factor.abs() <= tol.confusion() {
            og_bail!(Construction, "scale factor {factor} is degenerate");
        }
    ...
    pub fn apply_direction(&self, d: Direction, tol: Tolerances) -> OgResult<Direction> {
        match self.kind {
    ...
    pub fn apply_frame(&self, f: &Frame, tol: Tolerances) -> OgResult<Frame> {
        Frame::from_axes(
            self.apply(f.origin()),
    ...
    /// # Errors
    ///
    /// [`OgError::Numeric`](og_core::OgError::Numeric) if the linear part is
    ...
    pub fn inverse(&self) -> OgResult<Self> {
        ...
        if self.scale == 0.0 {
            og_bail!(Numeric, "transform has a zero scale and no inverse");
        }
...
impl Transform2 {
    ...
    /// [`OgError::Construction`](og_core::OgError::Construction) if `factor` is
    /// degenerate.
    pub fn scaling(centre: Point2, factor: f64, tol: Tolerances) -> OgResult<Self> {
        if !factor.is_finite() || factor.abs() <= tol.confusion() {
            og_bail!(Construction, "scale factor {factor} is degenerate");
        }
        let c = centre.to_vector();
    ...
    /// # Errors
    ///
    /// [`OgError::Numeric`](og_core::OgError::Numeric) if the transform is
    /// degenerate.
    pub fn inverse(&self) -> OgResult<Self> {
        if self.kind == TransformKind::Identity {
            return Ok(Self::IDENTITY);
...
impl GeneralTransform {
    ...
    /// [`OgError::Numeric`](og_core::OgError::Numeric) if the linear part is
    /// singular.
    pub fn apply_normal(&self, n: Vector) -> OgResult<Vector> {
