...
impl Axis {
    ...
    /// coincide.
    pub fn through(from: Point, to: Point, tol: Tolerances) -> OgResult<Self> {
...
impl Axis2 {
    ...
    /// [`OgError::Construction`](og_core::OgError::Construction) if the points
    /// coincide.
    pub fn through(from: Point2, to: Point2, tol: Tolerances) -> OgResult<Self> {
...
impl Frame {
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if `z` and
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if the three
    ...
    pub fn from_axes(
        ...
        for (a, b, names) in [(x, y, "x/y"), (y, z, "y/z"), (z, x, "z/x")] {
            if !a.is_normal(b, tol) {
                og_bail!(Construction, "frame axes {names} are not perpendicular");
            }
        }
        let triple = x.vector().triple(y.vector(), z.vector());
        if triple.abs() <= tol.angular() {
            og_bail!(Construction, "frame axes are coplanar");
        }
...
impl Frame2 {
    ...
    pub fn from_axes(
        ...
        if !x.is_normal(y, tol) {
            og_bail!(Construction, "frame axes are not perpendicular");
        }
