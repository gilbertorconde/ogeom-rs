...
impl KnotVector {
    ...
    /// sharp.
    pub fn new(knots: Vec<f64>, degree: usize) -> OgResult<Self> {
        if degree == 0 {
            og_bail!(Construction, "degree must be at least 1");
        }
        ...
        if !knots.iter().all(|k| k.is_finite()) {
            og_bail!(Construction, "knot vector contains a non-finite value");
        }
        if knots.windows(2).any(|w| w[1] < w[0]) {
            og_bail!(Construction, "knot vector is not non-decreasing");
        }

        let this = Self { knots, degree };
        if this.domain_start() >= this.domain_end() {
            og_bail!(Construction, "knot vector spans an empty domain");
        }
    ...
    pub fn clamped_uniform(degree: usize, control_points: usize) -> OgResult<Self> {
    ...
    pub fn averaged(degree: usize, parameters: &[f64]) -> OgResult<Self> {
        ...
        if parameters.windows(2).any(|w| w[1] <= w[0]) {
            og_bail!(Construction, "parameters must be strictly increasing");
        }
    ...
    pub fn span(&self, u: f64, tol: Tolerances) -> OgResult<usize> {
        let (start, end) = self.domain();
        if !u.is_finite() || u < start - tol.parametric() || u > end + tol.parametric() {
            og_bail!(Domain, "parameter {u} outside knot domain [{start}, {end}]");
        }
    ...
    pub fn with_knot_inserted(&self, value: f64, count: usize) -> OgResult<Self> {
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if the target
    /// range is empty or non-finite.
    pub fn reparameterized(&self, start: f64, end: f64) -> OgResult<Self> {
        if !start.is_finite() || !end.is_finite() || end <= start {
            og_bail!(Construction, "target range [{start}, {end}] is empty");
        }
        let (a, b) = self.domain();
