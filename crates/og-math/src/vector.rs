...
impl Vector {
    ...
    /// # Errors
    ///
    /// [`OgError::Range`](og_core::OgError::Range) if `index >= 3`.
    pub fn coord(self, index: usize) -> OgResult<f64> {
        match index {
            0 => Ok(self.x),
            1 => Ok(self.y),
            2 => Ok(self.z),
            _ => og_bail!(Range, "vector component {index} of 3"),
        }
    }
    ...
    pub fn normalized(self, tol: Tolerances) -> OgResult<Self> {
        if !self.is_finite() {
            og_bail!(Construction, "cannot normalize a non-finite vector");
        }
        let m = self.magnitude();
        if m <= tol.confusion() {
            og_bail!(Construction, "cannot normalize a vector of magnitude {m}");
        }
        Ok(self / m)
    }
    ...
    pub fn angle(self, other: Self, tol: Tolerances) -> OgResult<f64> {
        if self.is_zero(tol) || other.is_zero(tol) {
            og_bail!(Construction, "angle is undefined for a null vector");
        }
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if any vector
    /// is degenerate, or if `reference` is not perpendicular to both within
    /// `tol.angular()`.
    pub fn signed_angle(self, other: Self, reference: Self, tol: Tolerances) -> OgResult<f64> {
        ...
        if reference.is_zero(tol) {
            og_bail!(Construction, "reference vector is null");
        }
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if `other` is
    /// degenerate.
    pub fn projected_onto(self, other: Self, tol: Tolerances) -> OgResult<Self> {
        let square = other.square_magnitude();
        if square <= tol.confusion() * tol.confusion() {
            og_bail!(Construction, "cannot project onto a null vector");
        }
