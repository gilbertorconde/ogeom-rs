...
impl Point {
    ...
    pub fn coord(self, index: usize) -> OgResult<f64> {
        match index {
            ...
            1 => Ok(self.y),
            2 => Ok(self.z),
            _ => og_bail!(Range, "point coordinate {index} of 3"),
        }
    }
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if the set is
    /// empty.
    pub fn centroid(points: &[Self]) -> OgResult<Self> {
        let Some((first, rest)) = points.split_first() else {
            og_bail!(Construction, "centroid of an empty point set");
        };
