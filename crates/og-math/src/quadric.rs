...
fn check_positive(name: &str, value: f64, tol: Tolerances) -> OgResult<()> {
    if !value.is_finite() || value <= tol.confusion() {
        og_bail!(Construction, "{name} {value} must be finite and positive");
    }
    Ok(())
}
...
impl Plane {
    ...
    /// are collinear.
    pub fn through_points(a: Point, b: Point, c: Point, tol: Tolerances) -> OgResult<Self> {
    ...
    pub fn transformed(&self, t: &Transform, tol: Tolerances) -> OgResult<Self> {
...
impl Cylinder {
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if `radius` is
    /// not finite and positive.
    pub fn new(frame: Frame, radius: f64, tol: Tolerances) -> OgResult<Self> {
    ...
    ///
    /// As [`Cylinder::new`].
    pub fn about(axis: Axis, radius: f64, tol: Tolerances) -> OgResult<Self> {
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if `p` lies on
    /// the axis, where the nearest point — and so the normal — is not unique.
    pub fn normal_at(&self, p: Point, tol: Tolerances) -> OgResult<Direction> {
        let axis = self.axis();
...
impl Cone {
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if
...
impl Sphere {
    ...
    ///
    /// As [`Sphere::new`].
    pub fn centred(centre: Point, radius: f64, tol: Tolerances) -> OgResult<Self> {
...
/// # Errors
///
/// [`OgError::Construction`](og_core::OgError::Construction) if `a` and `b` are
/// collinear.
pub fn plane_normal(a: Vector, b: Vector, tol: Tolerances) -> OgResult<Direction> {
