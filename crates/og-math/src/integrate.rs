...
/// # Errors
///
/// [`OgError::Domain`](og_core::OgError::Domain) if the interval is not finite;
/// [`OgError::NotDone`](og_core::OgError::NotDone) if some part of it did not
...
pub fn integrate<F: FnMut(f64) -> f64>(mut f: F, a: f64, b: f64, tolerance: f64) -> OgResult<f64> {
    if !a.is_finite() || !b.is_finite() {
        og_bail!(Domain, "cannot integrate over [{a}, {b}]");
    }
    if !tolerance.is_finite() || tolerance <= 0.0 {
        og_bail!(Domain, "integration tolerance {tolerance} must be positive");
    }
    if a == b {
        return Ok(0.0);
    }
