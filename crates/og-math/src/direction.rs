...
impl Direction {
    ...
    pub fn new(v: Vector, tol: Tolerances) -> OgResult<Self> {
    ...
    ///
    /// As [`Direction::new`].
    pub fn from_coords(x: f64, y: f64, z: f64, tol: Tolerances) -> OgResult<Self> {
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if this
    /// direction is parallel to Z, leaving nothing to project.
    pub fn to_2d(self, tol: Tolerances) -> OgResult<Direction2> {
...
impl Direction2 {
    ...
    /// [`OgError::Construction`](og_core::OgError::Construction) if `v` is
    /// non-finite or shorter than `tol.confusion()`.
    pub fn new(v: Vector2, tol: Tolerances) -> OgResult<Self> {
        Ok(Self(v.normalized(tol)?))
    }
