...
//! in `og-math`; the spline curve carries its own control points. All of them
...
impl LineCurve {
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if the points
    /// coincide.
    pub fn segment(from: Point, to: Point, tol: Tolerances) -> OgResult<Self> {
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if the range
    /// is empty or non-finite.
    pub fn over(axis: Axis, start: f64, end: f64) -> OgResult<Self> {
        if !start.is_finite() || !end.is_finite() || end <= start {
            og_bail!(Construction, "line range [{start}, {end}] is empty");
        }
        Ok(Self {
            axis,
            domain: (start, end),
...
impl HyperbolaCurve {
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if `extent` is
    /// not finite and positive.
    pub fn new(hyperbola: Hyperbola, extent: f64) -> OgResult<Self> {
...
impl ParabolaCurve {
    ...
    /// [`OgError::Construction`](og_core::OgError::Construction) if `extent` is
    /// not finite and positive.
    pub fn new(parabola: Parabola, extent: f64) -> OgResult<Self> {
        if !extent.is_finite() || extent <= 0.0 {
            og_bail!(
                Construction,
...
impl BSplineCurve {
    ...
    pub fn new(knots: KnotVector, control: Vec<Point>, tol: Tolerances) -> OgResult<Self> {
    ...
    /// [`OgError::Dimension`](og_core::OgError::Dimension) if the control point
    /// count disagrees with the knot vector.
    pub fn rational(knots: KnotVector, control: Vec<Weighted<Point>>) -> OgResult<Self> {
    ...
    pub fn with_knot_inserted(&self, u: f64, count: usize, tol: Tolerances) -> OgResult<Self> {
    ...
    pub fn elevated(&self, tol: Tolerances) -> OgResult<Self> {
    ...
    ///
    /// As [`bspline::split`].
    pub fn split_at(&self, u: f64, tol: Tolerances) -> OgResult<(Self, Self)> {
...
impl TrimmedCurve {
    ...
    /// # Errors
    ///
    /// [`OgError::Domain`](og_core::OgError::Domain) if the range is empty or
    /// falls outside the basis curve's own domain.
    pub fn new(basis: Curve, start: f64, end: f64, tol: Tolerances) -> OgResult<Self> {
        let (a, b) = basis.domain();
        if !start.is_finite() || !end.is_finite() || end <= start + tol.parametric() {
            og_bail!(Domain, "trim range [{start}, {end}] is empty");
        }
    ...
    fn basis_parameter(&self, u: f64, tol: Tolerances) -> OgResult<f64> {
