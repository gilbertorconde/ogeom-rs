...
fn check_range(name: &str, lo: f64, hi: f64) -> OgResult<()> {
...
impl PlaneSurface {
    ...
    /// range is empty.
    pub fn over(plane: Plane, u: (f64, f64), v: (f64, f64)) -> OgResult<Self> {
...
impl CylinderSurface {
    ...
    /// is empty.
    pub fn new(cylinder: Cylinder, height: (f64, f64)) -> OgResult<Self> {
...
impl ConeSurface {
    ...
    /// [`OgError::Construction`](og_core::OgError::Construction) if the range
    /// is empty.
    pub fn new(cone: Cone, height: (f64, f64)) -> OgResult<Self> {
        check_range("height", height.0, height.1)?;
        Ok(Self { cone, height })
    }
...
impl RevolutionSurface {
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if the angle
    /// is not finite and positive, or exceeds a full turn.
    pub fn new(curve: Curve, axis: Axis, angle: f64) -> OgResult<Self> {
...
impl ExtrusionSurface {
    ...
    /// # Errors
    ///
    /// [`OgError::Construction`](og_core::OgError::Construction) if `distance`
    /// is not finite and positive.
    pub fn new(curve: Curve, direction: Direction, distance: f64) -> OgResult<Self> {
...
impl TrimmedSurface {
    ...
    /// # Errors
    ///
    /// [`OgError::Domain`](og_core::OgError::Domain) if either range is empty
    ...
    pub fn new(
        ...
        if !basis.is_periodic_u() && (u.0 < ua - eps || u.1 > ub + eps) {
            og_bail!(Domain, "u range [{}, {}] leaves [{ua}, {ub}]", u.0, u.1);
        }
        if !basis.is_periodic_v() && (v.0 < va - eps || v.1 > vb + eps) {
            og_bail!(Domain, "v range [{}, {}] leaves [{va}, {vb}]", v.0, v.1);
        }
        Ok(Self {
            basis,
            domain: (u, v),
...
mod tests {
    ...
    fn analytic_surfaces_contain_their_own_points() {
        // Cross-check against the independent distance functions in og-math.
