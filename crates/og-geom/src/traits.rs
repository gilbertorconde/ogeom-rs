...
use og_math::{Direction, Point, Point2, Transform, Vector, Vector2};
...
pub trait Curve3d {
    ...
    /// # Errors
    ///
    /// [`OgError::Domain`](og_core::OgError::Domain) if `u` is outside the
    /// domain and the curve is not periodic.
    fn point_at(&self, u: f64, tol: Tolerances) -> OgResult<Point>;
    ...
    ///
    /// As [`Curve3d::point_at`].
    fn d1_at(&self, u: f64, tol: Tolerances) -> OgResult<Vector>;
    ...
    ///
    /// As [`Curve3d::point_at`].
    fn derivatives_at(&self, u: f64, n: usize, tol: Tolerances) -> OgResult<Vec<Vector>>;
    ...
    fn tangent_at(&self, u: f64, tol: Tolerances) -> OgResult<Direction> {
    ...
    ///
    /// As [`Curve3d::point_at`].
    fn curvature_at(&self, u: f64, tol: Tolerances) -> OgResult<f64> {
    ...
    ///
    /// As [`Curve3d::point_at`].
    fn start(&self, tol: Tolerances) -> OgResult<Point> {
    ...
    ///
    /// As [`Curve3d::point_at`].
    fn end(&self, tol: Tolerances) -> OgResult<Point> {
    ...
    fn normalize_parameter(&self, u: f64, tol: Tolerances) -> OgResult<f64> {
        let (a, b) = self.domain();
        if self.is_periodic() {
            let period = b - a;
...
pub trait Curve2d {
    ...
    /// [`OgError::Domain`](og_core::OgError::Domain) if `u` is outside the
    /// domain and the curve is not periodic.
    fn point_at(&self, u: f64, tol: Tolerances) -> OgResult<Point2>;
    ...
    ///
    /// As [`Curve2d::point_at`].
    fn d1_at(&self, u: f64, tol: Tolerances) -> OgResult<Vector2>;
    ...
    ///
    /// As [`Curve2d::point_at`].
    fn derivatives_at(&self, u: f64, n: usize, tol: Tolerances) -> OgResult<Vec<Vector2>>;
    ...
    ///
    /// As [`Curve2d::point_at`].
    fn start(&self, tol: Tolerances) -> OgResult<Point2> {
        self.point_at(self.domain().0, tol)
    }
    ...
    ///
    /// As [`Curve2d::point_at`].
    fn end(&self, tol: Tolerances) -> OgResult<Point2> {
        self.point_at(self.domain().1, tol)
    }
}
...
pub trait Surface {
    ...
    /// # Errors
    ///
    /// [`OgError::Domain`](og_core::OgError::Domain) if the parameters lie
    /// outside the domain in a direction that is not periodic.
    fn point_at(&self, u: f64, v: f64, tol: Tolerances) -> OgResult<Point>;
    ...
    ///
    /// As [`Surface::point_at`].
    fn d1_at(&self, u: f64, v: f64, tol: Tolerances) -> OgResult<(Vector, Vector)>;
    ...
    ///
    /// As [`Surface::point_at`].
    fn d2_at(&self, u: f64, v: f64, tol: Tolerances) -> OgResult<(Vector, Vector, Vector)>;
    ...
    fn normal_at(&self, u: f64, v: f64, tol: Tolerances) -> OgResult<Direction> {
    ...
    ///
    /// As [`Surface::point_at`].
    fn is_degenerate_at(&self, u: f64, v: f64, tol: Tolerances) -> OgResult<bool> {
        let (du, dv) = self.d1_at(u, v, tol)?;
        let scale = du.magnitude().max(dv.magnitude());
    ...
    /// # Errors
    ///
    /// [`OgError::Domain`](og_core::OgError::Domain) if a parameter is outside
    /// a non-periodic direction's domain by more than `tol.parametric()`.
    fn normalize_parameters(&self, u: f64, v: f64, tol: Tolerances) -> OgResult<(f64, f64)> {
        let ((ua, ub), (va, vb)) = self.domain();
        let fix = |t: f64, a: f64, b: f64, periodic: bool, name: &str| -> OgResult<f64> {
            if periodic {
...
pub trait Transformable: Sized {
    ...
    fn transformed(&self, t: &Transform, tol: Tolerances) -> OgResult<Self>;
}
