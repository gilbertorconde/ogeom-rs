...
impl Line2d {
    ...
    /// coincide.
    pub fn segment(from: Point2, to: Point2, tol: Tolerances) -> OgResult<Self> {
        Ok(Self {
    ...
    pub fn over(axis: Axis2, start: f64, end: f64) -> OgResult<Self> {
...
impl BSpline2d {
    ...
    pub fn new(knots: KnotVector, control: Vec<Point2>, tol: Tolerances) -> OgResult<Self> {
    ...
    ///
    /// [`OgError::Dimension`](og_core::OgError::Dimension) on a shape mismatch.
    pub fn rational(knots: KnotVector, control: Vec<Weighted<Point2>>) -> OgResult<Self> {
...
impl Trimmed2d {
    ...
    pub fn new(basis: PlanarCurve, start: f64, end: f64, tol: Tolerances) -> OgResult<Self> {
        ...
        if !basis.is_periodic() && (start < a - tol.parametric() || end > b + tol.parametric()) {
            og_bail!(Domain, "trim range [{start}, {end}] leaves [{a}, {b}]");
        }
        Ok(Self {
            basis,
            domain: (start, end),
...
fn clamp_to_domain(u: f64, domain: (f64, f64), periodic: bool, tol: Tolerances) -> OgResult<f64> {
    let (a, b) = domain;
    if periodic {
        return Ok(a + (u - a).rem_euclid(b - a));
    }
    if !u.is_finite() || u < a - tol.parametric() || u > b + tol.parametric() {
        og_bail!(Domain, "parameter {u} outside curve domain [{a}, {b}]");
    }
    Ok(u.clamp(a, b))
}
...
/// # Errors
///
/// [`OgError::Construction`](og_core::OgError::Construction) at a cusp, where
/// the derivative vanishes.
pub fn tangent_angle(curve: &PlanarCurve, t: f64, tol: Tolerances) -> OgResult<f64> {
    let d = curve.d1_at(t, tol)?;
    if d.is_zero(tol) {
        og_bail!(Construction, "curve has no tangent at {t}");
    }
