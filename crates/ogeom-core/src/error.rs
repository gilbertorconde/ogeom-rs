...
pub type OgeomResult<T> = Result<T, OgeomError>;
...
///
/// ```
/// # use ogeom_core::{ogeom_err, OgeomError};
...
macro_rules! ogeom_err {
    ($variant:ident, $msg:literal) => {
        ...
        $crate::OgeomError::$variant($crate::Cause::from(format_args!($msg)))
    };
    ($variant:ident, $fmt:literal, $($arg:tt)*) => {
        $crate::OgeomError::$variant($crate::Cause::from(format_args!($fmt, $($arg)*)))
    };
}
...
macro_rules! ogeom_bail {
    ($variant:ident, $($arg:tt)*) => {
        return Err($crate::ogeom_err!($variant, $($arg)*))
    };
}
...
mod tests {
    ...
    fn static_cause_does_not_allocate() {
        let e = ogeom_err!(Domain, "parameter outside curve range");
        assert!(matches!(e, OgeomError::Domain(Cause::Static(_))));
    }
    ...
    fn inline_captures_interpolate_in_the_single_argument_form() {
        ...
        assert!(matches!(e, OgeomError::Range(Cause::Owned(_))));
    }

    #[test]
    fn formatted_cause_renders() {
        let e = ogeom_err!(Range, "index {} of {}", 7, 3);
        assert_eq!(e.to_string(), "out of range: index 7 of 3");
    }

    #[test]
    fn tolerance_sensitivity_splits_input_errors_from_algorithm_errors() {
        assert!(ogeom_err!(Numeric, "no convergence").is_tolerance_sensitive());
        assert!(ogeom_err!(NotDone, "could not close shell").is_tolerance_sensitive());
        // Retrying a degenerate construction with a looser tolerance is pointless.
        assert!(!ogeom_err!(Construction, "collinear points").is_tolerance_sensitive());
        assert!(!OgeomError::Cancelled.is_tolerance_sensitive());
    }

    #[test]
    fn bail_returns_early() {
        fn f(ok: bool) -> OgeomResult<u8> {
            if !ok {
                ogeom_bail!(NullObject, "no shape");
            }
            Ok(1)
        }
