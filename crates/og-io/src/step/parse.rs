...
/// # Errors
///
/// [`OgError::Construction`](og_core::OgError::Construction) on malformed
/// syntax, with the byte offset where reading stopped making sense.
pub fn parse(text: &str) -> OgResult<Exchange> {
    let mut p = Parser {
        bytes: text.as_bytes(),
        at: 0,
    };
...
impl Parser<'_> {
    ...
    fn expect(&mut self, byte: u8) -> OgResult<()> {
        self.skip_noise();
    ...
    fn expect_keyword(&mut self, word: &str) -> OgResult<()> {
    ...
    fn keyword(&mut self) -> OgResult<String> {
        self.skip_noise();
        let start = self.at;
        while self
            .bytes
    ...
    fn integer(&mut self) -> OgResult<i64> {
        self.skip_noise();
        let start = self.at;
    ...
    fn arguments(&mut self) -> OgResult<Vec<Arg>> {
        self.expect(b'(')?;
        let mut out = Vec::new();
        loop {
            self.skip_noise();
    ...
    fn argument(&mut self) -> OgResult<Arg> {
        self.skip_noise();
        let Some(&byte) = self.bytes.get(self.at) else {
            og_bail!(Construction, "the exchange file ends inside an argument");
        };
        match byte {
            b'$' => {
                self.at += 1;
    ...
    fn string(&mut self) -> OgResult<Arg> {
        self.expect(b'\'')?;
        let mut out = String::new();
        loop {
            match self.bytes.get(self.at) {
                None => og_bail!(Construction, "the exchange file ends inside a string"),
                Some(b'\'') => {
    ...
    fn number(&mut self) -> OgResult<Arg> {
        let start = self.at;
